namespace Loupedeck.ExamplePlugin
{
    ...
    public class ActionMeta5
    {
        public string Title { get; set; } = "";
        public string IconPath { get; set; } = ""; 
    }

    public class DynamicCommand5 : PluginDynamicCommand
    {
        ...
        private string ScriptPath => Path.Combine(BasePath, "script_5.py");
        private string MetaPath => Path.Combine(BasePath, "script_5.json");
        private string DebugLogPath = "/tmp/MX_DEBUG.txt";

        // Variabili di stato
        private ActionMeta5 _currentMeta = new ActionMeta5();
        private FileSystemWatcher _watcher;

        public DynamicCommand5()
            : base(displayName: "Dynamic AI Action 5", description: "Action updated by Python", groupName: "AI Commands")
        {
            ...
            if (Directory.Exists(BasePath))
            {
                try 
                {
                    ...
                    _watcher.Filter = "script_5.json"; // Ascolta solo il file di metadati
        ...
        // Evento: Scatta quando Python modifica script_5.json
        ...
        private void UpdateMetaFromFile()
        {
            try
            {
                if (File.Exists(MetaPath))
                {
                    ...
                    var meta = JsonConvert.DeserializeObject<ActionMeta5>(jsonContent);
