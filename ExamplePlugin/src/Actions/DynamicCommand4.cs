namespace Loupedeck.ExamplePlugin
{
    ...
    public class ActionMeta4
    {
        public string Title { get; set; } = "";
        public string IconPath { get; set; } = ""; 
    }

    public class DynamicCommand4 : PluginDynamicCommand
    {
        ...
        private string ScriptPath => Path.Combine(BasePath, "script_4.py");
        private string MetaPath => Path.Combine(BasePath, "script_4.json");
        private string DebugLogPath = "/tmp/MX_DEBUG.txt";

        // Variabili di stato
        private ActionMeta4 _currentMeta = new ActionMeta4();
        private FileSystemWatcher _watcher;

        public DynamicCommand4()
            : base(displayName: "Dynamic AI Action 4", description: "Action updated by Python", groupName: "AI Commands")
        {
            ...
            if (Directory.Exists(BasePath))
            {
                try 
                {
                    ...
                    _watcher.Filter = "script_4.json"; // Ascolta solo il file di metadati
        ...
        // Evento: Scatta quando Python modifica script_4.json
        ...
        private void UpdateMetaFromFile()
        {
            try
            {
                if (File.Exists(MetaPath))
                {
                    ...
                    var meta = JsonConvert.DeserializeObject<ActionMeta4>(jsonContent);
