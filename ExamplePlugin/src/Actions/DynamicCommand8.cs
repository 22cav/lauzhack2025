namespace Loupedeck.ExamplePlugin
{
    ...
    public class ActionMeta8
    {
        public string Title { get; set; } = "";
        public string IconPath { get; set; } = ""; 
    }

    public class DynamicCommand8 : PluginDynamicCommand
    {
        ...
        private string ScriptPath => Path.Combine(BasePath, "script_8.py");
        private string MetaPath => Path.Combine(BasePath, "script_8.json");
        private string DebugLogPath = "/tmp/MX_DEBUG.txt";

        // Variabili di stato
        private ActionMeta8 _currentMeta = new ActionMeta8();
        private FileSystemWatcher _watcher;

        public DynamicCommand8()
            : base(displayName: "Dynamic AI Action 8", description: "Action updated by Python", groupName: "AI Commands")
        {
            ...
            if (Directory.Exists(BasePath))
            {
                try 
                {
                    ...
                    _watcher.Filter = "script_8.json"; // Ascolta solo il file di metadati
        ...
        // Evento: Scatta quando Python modifica script_8.json
        ...
        private void UpdateMetaFromFile()
        {
            try
            {
                if (File.Exists(MetaPath))
                {
                    ...
                    var meta = JsonConvert.DeserializeObject<ActionMeta8>(jsonContent);
