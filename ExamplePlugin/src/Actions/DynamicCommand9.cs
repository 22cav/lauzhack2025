namespace Loupedeck.ExamplePlugin
{
    ...
    public class ActionMeta9
    {
        public string Title { get; set; } = "";
        public string IconPath { get; set; } = ""; 
    }

    public class DynamicCommand9 : PluginDynamicCommand
    {
        ...
        private string ScriptPath => Path.Combine(BasePath, "script_9.py");
        private string MetaPath => Path.Combine(BasePath, "script_9.json");
        private string DebugLogPath = "/tmp/MX_DEBUG.txt";

        // Variabili di stato
        private ActionMeta9 _currentMeta = new ActionMeta9();
        private FileSystemWatcher _watcher;

        public DynamicCommand9()
            : base(displayName: "Dynamic AI Action 9", description: "Action updated by Python", groupName: "AI Commands")
        {
            ...
            if (Directory.Exists(BasePath))
            {
                try 
                {
                    ...
                    _watcher.Filter = "script_9.json"; // Ascolta solo il file di metadati
        ...
        // Evento: Scatta quando Python modifica script_9.json
        ...
        private void UpdateMetaFromFile()
        {
            try
            {
                if (File.Exists(MetaPath))
                {
                    ...
                    var meta = JsonConvert.DeserializeObject<ActionMeta9>(jsonContent);
